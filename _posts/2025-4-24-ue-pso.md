...
r.PSOPrecache.BreakOnMaterialName 在 PSO 预处理过程中遇到具有给定名称的材料时很有用 —— 这可以帮助找出为什么某些渲染状态与运行时状态不同。r.PSOPrecache.BreakOnPassName 和 r.PSOPrecache.BreakOnShaderHash 也可以用来缩小有问题的 PSO。如上所述，这些信息可以在日志中找到。
r.PSOprecache.UseBackgroundThreadForCollection 有助于禁用 PSO 初始化器集合的后台线程任务，以便在调试 PSO 预防错误时更容易跟踪组件信息或其他状态。

tips:你可能还需要检查 FPSOPrecacheParams 的值，因为这些值也可能影响 PSO 中使用的着色器和渲染状态。
